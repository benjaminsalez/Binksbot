// Dictionnaire FR -> EN COMPLET des noms de Pokemon (generations 1 a 8,
// genere depuis la base communautaire veekun/pokedex).
// La generation 9 (Ecarlate/Violet, 2022+) n'y figure pas encore, a
// completer manuellement si besoin plus tard.
 ...
 * pourrait deja etre en anglais, ou correspondre a la generation 9 non couverte).
 */
...
// imprimee en anglais. On exclut volontairement les noms identiques dans
// les deux langues (Pikachu, Onix, Mewtwo...) car ils ne donnent aucun
// signal fiable.
export const EN_NAMES_DIFFERING_FROM_FR = new Set(
  Object.entries(FR_TO_EN)
    .filter(([fr, en]) => fr.toLowerCase() !== en.toLowerCase())
    .map(([, en]) => en.toLowerCase())
);

/**
 * Verifie si un nom devine correspond a un nom anglais "trahissant" une
 * carte non-francaise (voir EN_NAMES_DIFFERING_FROM_FR ci-dessus).
 */
export function looksLikeEnglishCardName(guessedName) {
  if (!guessedName) return false;
  return EN_NAMES_DIFFERING_FROM_FR.has(guessedName.toLowerCase().trim());
}
