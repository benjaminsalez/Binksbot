...
export async function checkIfGoodDeal(item, thresholdPercent) {
  ...
  return {
    ...
    // RECHERCHE Pokecardex (site francophone de reference). On inclut le
    // numero de carte quand on l'a, pour restreindre fortement les
    // resultats (une recherche par nom seul montre TOUTES les versions).
    cardmarketSearchUrl:
      cardmarketDirectUrl ||
      `https://www.pokecardex.com/search?q=${encodeURIComponent(
        setNumber ? `${matchedName} ${setNumber}` : matchedName
      )}`,
...
export async function checkIfGoodDeal(item, thresholdPercent) {
  ...
  return {
    ...
    // Lien vers Cardmarket: le lien DIRECT vers la fiche exacte si
    // pokemontcg.io a repondu (rare mais precis), sinon un lien de
    // RECHERCHE (toujours disponible mais moins precis).
    cardmarketSearchUrl:
      cardmarketDirectUrl ||
      `https://www.cardmarket.com/fr/Pokemon/Products/Search?searchString=${encodeURIComponent(matchedName)}`,
    cardmarketUrlIsExact: Boolean(cardmarketDirectUrl),
    favouriteCount: item.favouriteCount,
    viewCount: item.viewCount,
  };
